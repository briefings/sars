...
class HPG:
    ...
    """
    The grid of Hospitalized/100K [C] v Positives/100K [C] Graphs
    """

    def __init__(self, hospitalized_rate_max: float, positive_rate_max: float):
        ...
        self.gradients = np.concatenate((np.linspace(start=0, stop=0.3, num=3, endpoint=False),
                                         ...
                                         np.arange(1, 10), np.arange(10, 80, 10)),
                                        axis=0) / 100
    ...
    def grid(self, abscissae: np.ndarray) -> pd.DataFrame:
        ...
        # Renaming fields
        option = lines.rename(columns={'x': 'positiveRate', 'y': 'hospitalizedRate'})

        # An inexistent state code -> a placeholder
        option.loc[:, 'STUSPS'] = 'ZZ'
        option = option[option['hospitalizedRate'] <= self.ylimit]
